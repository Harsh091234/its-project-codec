...
import ConfirmDeletePanel from "./panels/ConfirmDeletePanel";
    ...
    const csvContent =
      ...
      ["ID,FullName,Email,Gender,Status"]
        .concat(
          filteredUsers.map(
            (u) =>
              `${u._id},${u.firstName + u.lastName},${u.email},${u.gender},${u.status}`,
          ),
        )
        .join("\n");
      ...
      {showConfirm && (
        <ConfirmDeletePanel
          onConfirm={async () => {
            ...
            try {
              await deleteUser(id).unwrap(); // ✅ call unwrap()
              ...
              {filteredUsers.map((user) => (
                ...
                >
                  ...
                  <td className="px-2 py-3">
                    <img
                      src={
                        user.profileImage || "https://i.pravatar.cc/40?img=1"
                      }
